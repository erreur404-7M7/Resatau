...
def search(request):
    ...
    if len(allProds) == 0 or len(query) < 3:
        darshan = {'msg': "Aucun article disponible. Veuillez vous assurer de saisir une requête de recherche pertinente."}
...
def checkout(request):
    if request.method == "POST":
        ...
        order.save()
        update = OrderUpdate(order_id=order.order_id, update_desc="La commande a été passée.")
        update.save()
        thank = True
        id = order.order_id
...
def handeLogin(request):
    if request.method == "POST":
        ...
        if user is not None:
            ...
            messages.warning(request, "Identifiants non valides ! Veuillez réessayer.")
            return HttpResponseRedirect(request.META.get('HTTP_REFERER'))
...
def handleSignUp(request):
    if request.method == "POST":
        ...
        if (password1 != password):
            messages.warning(request, " Le mot de passe ne correspondt pas.")
            return HttpResponseRedirect(request.META.get('HTTP_REFERER'))

        try:
            user = User.objects.get(username=username)
            messages.warning(request, " Nom d'utilisateur déjà pris. Essayez avec un nom d'utilisateur différent.")
            return HttpResponseRedirect(request.META.get('HTTP_REFERER'))
...
def handlerequest(request):
    ...
    if verify:
        if response_dict['RESPCODE'] == '01':
            ...
            print("La commande n'a pas abouti car" + response_dict['RESPMSG'])
